...
class SyncManager {
  ...
  async getState(): Promise<SyncState> {
    ...
    const pendingCount = userId
      ? await fileStorageService.getPendingCount(userId)
      : 0;
    const lastSyncTime = await fileStorageService.getLastSyncTime();
  ...
  private async performSyncInternal(userId: string): Promise<SyncResult> {
    try {
      const localEntries = await fileStorageService.getAllEntriesForSync(userId);
      ...
      await fileStorageService.replaceAllEntries(userId, entriesWithLocalImages);
      await fileStorageService.setLastSyncTime(result.timestamp);
  ...
  async quickSync(): Promise<SyncResult> {
    ...
    const pendingEntries = await fileStorageService.getPendingEntries(userId);
