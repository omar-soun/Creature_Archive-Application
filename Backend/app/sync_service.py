...
from .config import init_firebase, check_firestore_reachable
from .models import (
    SyncEntryData,
    SyncedEntryResponse,
    TwoWaySyncRequest,
...
def _entry_to_firestore(entry: SyncEntryData, image_url: str) -> dict:
    ...
    return {
        ...
        "imageUrl": image_url,
        "location": GeoPoint(entry.latitude, entry.longitude) if entry.latitude and entry.longitude else None,
        "locationName": entry.locationName or "",
        "capturedAt": datetime.fromtimestamp(entry.capturedAt / 1000, tz=timezone.utc),
        "createdAt": datetime.fromtimestamp(entry.createdAt / 1000, tz=timezone.utc),
        "lastUpdated": datetime.fromtimestamp(entry.lastUpdated / 1000, tz=timezone.utc),
        ...
        "localId": entry.localId,  # Store localId for cross-device identification
    }


def _doc_to_sync_response(doc, local_image_uri: str = "") -> SyncedEntryResponse:
    ...
    if hasattr(captured_at, 'timestamp'):
        ...
        captured_at_ms = int(captured_at.timestamp() * 1000) if captured_at else 0
    ...
    return SyncedEntryResponse(
        localId=data.get("localId", doc.id),  # Use stored localId or fallback to firestoreId
        firestoreId=doc.id,
        userId=data["userId"],
...
def _build_deferred_entries(request: TwoWaySyncRequest, auth_uid: str) -> list[SyncedEntryResponse]:
    ...
    for entry in request.localEntries:
        if entry.userId != auth_uid or entry.isDeleted:
            continue
        entries.append(SyncedEntryResponse(
            ...
            syncStatus=entry.syncStatus,  # Preserve current status
            notes=entry.notes,
            tags=entry.tags,
            behaviorObserved=entry.behaviorObserved,
            quantity=entry.quantity,
...
def perform_two_way_sync(request: TwoWaySyncRequest, auth_uid: str) -> TwoWaySyncResponse:
    """
    ...
    Args:
        request: TwoWaySyncRequest containing local entries and last sync time
        auth_uid: Authenticated user's UID

    Returns:
        TwoWaySyncResponse with merged entries and sync statistics
    """
    ...
    if not check_firestore_reachable():
        return TwoWaySyncResponse(
            ...
            mergedEntries=_build_deferred_entries(request, auth_uid),
            uploaded=0,
            downloaded=0,
            deleted=0,
            conflicts=0,
    ...
    try:
        ...
        for entry in request.localEntries:
            ...
            if entry.userId != auth_uid:
                result.errors.append(f"Entry {entry.localId} has mismatched userId")
                continue
        ...
        for entry in request.localEntries:
            ...
            if entry.isDeleted:
                if entry.firestoreId and entry.firestoreId in cloud_by_id:
                    try:
                        db.collection(COLLECTION).document(entry.firestoreId).delete()
                        result.deleted += 1
                        entry_count_delta -= 1
                        processed_firestore_ids.add(entry.firestoreId)
                    except Exception as e:
                        result.errors.append(f"Failed to delete {entry.firestoreId}: {str(e)}")
                elif entry.localId in cloud_by_local_id:
                    # Find by localId if firestoreId not set
                    try:
                        cloud_doc = cloud_by_local_id[entry.localId]
                        db.collection(COLLECTION).document(cloud_doc.id).delete()
                        result.deleted += 1
                        entry_count_delta -= 1
                        processed_firestore_ids.add(cloud_doc.id)
                        processed_local_ids.add(entry.localId)
                    except Exception as e:
                        result.errors.append(f"Failed to delete by localId {entry.localId}: {str(e)}")
                continue
            ...
            if cloud_doc:
                ...
                if hasattr(cloud_last_updated, 'timestamp'):
                    cloud_last_updated_ms = int(cloud_last_updated.timestamp() * 1000)
                else:
                    cloud_last_updated_ms = 0

                if entry.syncStatus == "pending" and entry.lastUpdated > cloud_last_updated_ms:
                    # Local is newer - update cloud
                    try:
                        image_url = entry.imageUrl or cloud_data.get("imageUrl", "")
                        update_data = {
                            "speciesName": entry.speciesName,
                            "scientificName": entry.scientificName,
                            "confidenceScore": entry.confidenceScore,
                        ...
                        db.collection(COLLECTION).document(cloud_doc.id).update(update_data)
                        result.uploaded += 1
                        ...
                        result.errors.append(f"Failed to update {entry.localId}: {str(e)}")
                        continue
                else:
                    # Cloud is newer or same - use cloud data
                    result.conflicts += 1
                    synced_entry = _doc_to_sync_response(cloud_doc, entry.localImageUri)
                    synced_entry.localId = entry.localId  # Preserve client's localId
                    result.mergedEntries.append(synced_entry)
                ...
                try:
                    ...
                    doc_data = _entry_to_firestore(entry, image_url)
                    ...
                    result.errors.append(f"Failed to upload {entry.localId}: {str(e)}")

            else:
                # Entry is synced locally but not in cloud - might have been deleted on another device
                # Include it anyway with synced status
                synced_entry = SyncedEntryResponse(
                    localId=entry.localId,
                    firestoreId=entry.firestoreId or "",
                    userId=entry.userId,
        ...
        for firestore_id, cloud_doc in cloud_by_id.items():
            ...
            synced_entry = _doc_to_sync_response(cloud_doc)
            result.mergedEntries.append(synced_entry)
            result.downloaded += 1
        ...
        if entry_count_delta != 0:
            try:
                ...
                user_ref.update({
                    "entryCount": _firestore_increment(entry_count_delta),
                    "lastSync": now,
                })
            except Exception as e:
                result.errors.append(f"Failed to update user entry count: {str(e)}")
        else:
